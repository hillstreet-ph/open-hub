...
| PostgreSQL schema | `open_hub` |
| Sentry organization | `hillstreet` |
| Sentry project | `open-hub` |
| GitHub repository | `hillstreet-ph/open-hub` |
| Container image | `${DOCKERHUB_USERNAME}/open-hub` |
...
7. Supabase migrations must target schema `open_hub` and must be reviewed before production execution.
